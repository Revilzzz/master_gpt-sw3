...
# Ladda och förbered osuperviserad data från txt-filer
...
unsupervised_dataset = load_unsupervised_data('txtfiler')


full_dataset = DatasetDict({
    'train': Dataset.from_dict({'text': qa_dataset['text'] + unsupervised_dataset['text']})
})


dataset_split = full_dataset['train'].train_test_split(test_size=0.1)
...
# Träningsloop för både superviserad och osuperviserad data
print("Starting training with both supervised and unsupervised data...")
...
# Spara modellen
model.save_pretrained('./results16')
tokenizer.save_pretrained('./results16')
